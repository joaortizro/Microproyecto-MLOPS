...
run_name = (
    sys.argv[4]
    if len(sys.argv) > 4
    else f"lgbm-est{n_estimators}-d{max_depth}-lr{learning_rate}"
)
...
X = df_training[FEATURE_COLUMNS].fillna(
    df_training[FEATURE_COLUMNS].median()
)
...
mlflow.set_experiment("olist-negative-review-classif-LightGBM")
